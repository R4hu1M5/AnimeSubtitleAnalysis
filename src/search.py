...
def build(res, task_attrs, has_terms=False):
    print("Creating XML tree for the search results")
    # Different attributes and file path for Task 2 and Task 1/3
    if not has_terms:
        attrs = {"task": str(task_attrs), "key": "Untranslated Words"}
        path = f"results/task{task_attrs}.xml"
    else:
        task_num, term_num, term = task_attrs
        attrs = {"task": str(task_num), "term": str(term_num), "key": term}
        path = f"../results/task{task_num}_term{term_num}.xml"
    results = ET.Element("results", attrib=attrs)
...
def transform(task_attrs, has_terms=False):
    # Different file path for Task 2 and Task 1/3
    if not has_terms:
        partial_path = f"results/task{task_attrs}"
    else:
        task_num, term_num = task_attrs
        partial_path = f"results/task{task_num}_term{term_num}"
    print(f"Generating ../{partial_path}.html")
    xml = etree.parse(f"../{partial_path}.xml")
    xslt = etree.parse("../results/transform.xslt")
    transform = etree.XSLT(xslt)
    result = transform(xml)
    with open(f"../{partial_path}.html", "wb") as f:
